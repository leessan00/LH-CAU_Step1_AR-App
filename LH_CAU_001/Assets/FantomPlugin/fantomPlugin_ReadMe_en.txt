...
(*) In the build of Unity 2018.1.0, when "Build System" is set to "Gradle" in "Build Settings...", it is known that the package name is not added to "AndroidManifest.xml", and a build error occurs. In that case, please write "package="(package name of application)" (= Edit> Project Settings> Player> Other Settings> Identification> Package Name) in "manifest" tag of "AndroidManifest.xml" (Until Unity 2017.4.2 is added automatically).
Or you can build it by setting "Build System" to "Internal".
